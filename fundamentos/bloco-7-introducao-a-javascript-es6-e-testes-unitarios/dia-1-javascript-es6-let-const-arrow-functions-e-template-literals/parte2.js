...
const searchAndReplaceXLetter = (word) => {
...
const skillsArray = ["Javascript", "Git", "Html", "Css", "Flexbox"];

const concatSkills = () => {
  const sortSkillsArray = skillsArray.sort();
  let result = `${searchAndReplaceXLetter(
    "Danilo"
  )} Minhas cinco principais habilidades são:`;
  for (const skill of sortSkillsArray) {
    result += `
    - ${skill}`;
  }
  return result += `
